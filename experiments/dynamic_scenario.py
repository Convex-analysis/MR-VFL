    ...
    with open(os.path.join(output_dir, f"dynamic_scenario_summary_{timestamp}.txt"), "w") as f:
        ...
        if "Mamba" in results and "Transformer" in results:
            ...
            if transformer_adaptability > 0:
                ...
                # Handle the case where transformer_adaptability is zero
                if mamba_adaptability > 0:
                    f.write(f"\n## Mamba's Advantage\n\n")
                    f.write(f"- Adaptability advantage over Transformer: Infinite (Transformer adaptability is zero)\n")
                else:
                    f.write(f"\n## Mamba's Advantage\n\n")
                    f.write(f"- Adaptability advantage over Transformer: N/A (Both adaptabilities are zero)\n")
